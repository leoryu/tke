...
import (
	...
	"tkestack.io/tke/pkg/platform/registry/volumedecorator"
...
// Storage includes storage for VolumeDecorator and all sub resources.
type Storage struct {
	VolumeDecorator *REST
	Status          *StatusREST
}
...
func NewStorage(optsGetter genericregistry.RESTOptionsGetter, privilegedUsername string) *Storage {
	...
	store := &registry.Store{
		...
		DefaultQualifiedResource: platform.Resource("volumedecorators"),
		PredicateFunc:            volumedecorator.MatchVolumeDecorator,
	...
	if err := store.CompleteWithOptions(options); err != nil {
		log.Panic("Failed to create VolumeDecorator etcd rest storage", log.Err(err))
	}

	statusStore := *store
	statusStore.UpdateStrategy = volumedecorator.NewStatusStrategy(strategy)
	statusStore.ExportStrategy = volumedecorator.NewStatusStrategy(strategy)

	return &Storage{
		VolumeDecorator: &REST{store, privilegedUsername},
		Status:          &StatusREST{&statusStore},
	}
}

// ValidateGetObjectAndTenantID validate name and tenantID, if success return VolumeDecorator
func ValidateGetObjectAndTenantID(ctx context.Context, store *registry.Store, name string, options *metav1.GetOptions) (runtime.Object, error) {
	...
	decorator := obj.(*platform.VolumeDecorator)
	if err := util.FilterVolumeDecorator(ctx, decorator); err != nil {
		return nil, err
	}
	return decorator, nil
}

// ValidateExportObjectAndTenantID validate name and tenantID, if success return VolumeDecorator
...
// REST implements a RESTStorage for VolumeDecorator against etcd.
type REST struct {
	*registry.Store
...
func (r *REST) DeleteCollection(ctx context.Context, deleteValidation rest.ValidateObjectFunc, options *metav1.DeleteOptions, listOptions *metainternal.ListOptions) (runtime.Object, error) {
	if !authentication.IsAdministrator(ctx, r.privilegedUsername) {
		return nil, errors.NewMethodNotSupported(platform.Resource("volumedecorators"), "delete collection")
	}
	return r.Store.DeleteCollection(ctx, deleteValidation, options, listOptions)
}

// StatusREST implements the REST endpoint for changing the status of a VolumeDecorator.
